...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        choices=["similarity_only", "weighted_score", "rerank_similarity"],
    ...
    if fewshot_gate_enabled:
        ...
        if fewshot_gate_mode not in {"similarity_only", "weighted_score", "rerank_similarity"}:
            raise ValueError(
                "fewshot_gate_mode must be one of: similarity_only, weighted_score, "
                "rerank_similarity"
            )
        if fewshot_gate_mode in {"similarity_only", "weighted_score"} and fewshot_gate_threshold is None:
            raise ValueError(
                "fewshot_gate_threshold is required for similarity_only or weighted_score"
            )
    ...
    code_provenance_paths = {
        ...
        "retrieval_utils": project_root / "src" / "retrieval_utils.py",
    ...
    run_provenance: dict[str, Any] = {
        ...
        "base_model_revision": client.resolve_model_revision(llm),
    ...
    out_csv = results_dir / f"run_{adapter}_{timestamp}.csv"
    out_metadata_json = results_dir / f"run_{adapter}_{timestamp}_metadata.json"
    ...
    if fewshot_active:
        ...
        retrieval_trace_path = retrieval_trace_dir / f"run_{adapter}_{timestamp}_retrieval_traces.jsonl"
    ...
    run_metadata: dict[str, Any] = {
        ...
        "run_model_revision": client.resolve_model_revision(llm) or "",
        "run_adapter": adapter,
