...
K3_RUNNER_VERSION = "dynamic_k3_sqltimeout_v3"
...
def run_sql(
    ...
    def abort_after_deadline() -> int:
        nonlocal aborted_by_timeout
        if time.monotonic() >= deadline:
            aborted_by_timeout = True
            return 1
        return 0

    conn.set_progress_handler(abort_after_deadline, 10_000)
    try:
        cur = conn.cursor()
        cur.execute(sql)
        rows = cur.fetchall()
        ...
        if aborted_by_timeout:
            return ExecResult(
                ...
                error=(
                    "SQLExecutionTimeout("
                    f"limit_seconds={execution_timeout_seconds:g}, error={e!r})"
                ),
            )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Per-statement SQLite execution timeout enforced with a progress handler.",
    )
    parser.add_argument(
    ...
    execution_timeout_seconds = float(
        get_param(args, cfg, "execution_timeout_seconds", 900.0)
    )
    if not math.isfinite(execution_timeout_seconds) or execution_timeout_seconds <= 0:
        raise ValueError("execution_timeout_seconds must be finite and greater than zero")
    ...
    gold_sql_timeout_total = pred_sql_timeout_total = 0
    ...
    run_metadata: dict[str, Any] = {
        ...
        "run_execution_timeout_seconds": execution_timeout_seconds,
    ...
    with out_csv.open("w", encoding="utf-8", newline="") as csvfile:
        ...
        def write_case_result(record: dict[str, Any], generation_result: GenerationResult) -> None:
            ...
            nonlocal gold_sql_timeout_total, pred_sql_timeout_total
            ...
            if db_error is None and conn is not None:
                gold_res = run_sql(conn, gold_sql, execution_timeout_seconds)
                pred_res = (
                    run_sql(conn, pred_sql, execution_timeout_seconds)
                    if pred_sql
                    else ExecResult(False, None, "No SQL extracted")
                )
            else:
            ...
            if gold_res.ok:
                gold_exec_ok += 1
            elif (gold_res.error or "").startswith("SQLExecutionTimeout("):
                gold_sql_timeout_total += 1
                logger.error(
                    "Gold SQL execution timed out for id=%s after %ss",
                    qid,
                    f"{execution_timeout_seconds:g}",
                )

            if pred_res.ok:
                pred_exec_ok += 1
            elif (pred_res.error or "").startswith("SQLExecutionTimeout("):
                pred_sql_timeout_total += 1
                logger.warning(
                    "Predicted SQL execution timed out for id=%s after %ss; "
                    "recording prediction as non-executable",
                    qid,
                    f"{execution_timeout_seconds:g}",
                )
    ...
    run_summary_metadata = {
        **run_metadata,
        ...
        "gold_sql_timeout_total": gold_sql_timeout_total,
        "pred_sql_timeout_total": pred_sql_timeout_total,
    ...
    logger.info(
        "SQLite execution timeout: %ss per statement (gold_timeouts=%s, pred_timeouts=%s)",
        f"{execution_timeout_seconds:g}",
        gold_sql_timeout_total,
        pred_sql_timeout_total,
    )
